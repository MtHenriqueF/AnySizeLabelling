...
class ImageLabel(QLabel):
    ...
    def draw_annotations(self, painter):
        ...
        for class_name, class_annotations in self.annotations.items():
            # Provavelmente aqui farei algum tipo de check
            ...
            for annotation in class_annotations:
                ...
                if "segmentation" in annotation:
                    if not annotation.get('edit', False):
                        continue
    ...
    def wheelEvent(self, event: QWheelEvent):
        if event.modifiers() == Qt.ControlModifier:
            ...
            event.accept()  # Impede propagação do evento
        else:
            # Scroll normal → mover slider externo
            delta = event.angleDelta().y()
            step = 10

            if event.modifiers() & Qt.ShiftModifier:
                # Shift + Scroll → mover horizontal
                new_val = self.main_window.external_horizontal_slider.value()
                new_val += -step if delta > 0 else step
                self.main_window.external_horizontal_slider.setValue(new_val)
            else:
                # Scroll normal → mover vertical
                new_val = self.main_window.external_vertical_slider.value()
                new_val += -step if delta > 0 else step
                self.main_window.external_vertical_slider.setValue(new_val)

            event.accept()  # Bloqueia a propagação (e scroll interno)
    ...
    def mousePressEvent(self, event: QMouseEvent):
        ...
        if event.button() == Qt.RightButton:
            ...
            print(self.pan_start_pos)
            event.accept()
        else:
            pos = self.get_image_coordinates(event.pos())
            if event.button() == Qt.LeftButton:
                if self.sam_magic_wand_active and self.sam_mode == 'point':
                    self.sam_point = [pos[0], pos[1]]
                    self.main_window.apply_sam_prediction()
                if self.sam_magic_wand_active and self.sam_mode == 'box':
    ...
    def mouseMoveEvent(self, event: QMouseEvent):
        ...
        if event.buttons() == Qt.RightButton:
            if self.pan_start_pos:
                ...
                self.pan_start_pos = event.pos()
                # Atualiza os sliders externos com base no movimento
                h_slider = self.main_window.external_horizontal_slider
                v_slider = self.main_window.external_vertical_slider

                h_new = h_slider.value() - delta.x()
                v_new = v_slider.value() - delta.y()

                if h_new != h_slider.value():
                    h_slider.setValue(h_new)

                if v_new != v_slider.value():
                    v_slider.setValue(v_new)
            event.accept()
        else:
            pos = self.cursor_pos
