...
from .anysize.anysize_control import ImageController
from .anysize.anysize_annotations import AnnotationUtils
...
class WheelToExternalSlider(QObject):
    def __init__(self, vertical_slider, horizontal_slider):
        super().__init__()
        self.v_slider = vertical_slider
        self.h_slider = horizontal_slider

    def eventFilter(self, obj, event):
        if event.type() == QEvent.Wheel:
            print("chamando filtro do evento de roda")
            delta = event.angleDelta().y()
            step = self.v_slider.singleStep()

            if event.modifiers() & Qt.ShiftModifier:
                # Shift + roda → scroll horizontal
                new_val = self.h_slider.value() - step if delta > 0 else self.h_slider.value() + step
                self.h_slider.setValue(new_val)
            else:
                # Roda normal → scroll vertical
                new_val = self.v_slider.value() - step if delta > 0 else self.v_slider.value() + step
                self.v_slider.setValue(new_val)

            return True  # Bloqueia o scroll interno
        return False

class InfoPanel(QWidget):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.pos_label = QLabel("Top Left (x,y): (0, 0)")
        self.center_label = QLabel("Center: (0, 0)")
        self.size_label = QLabel("Crop: 0 x 0")
    ...
    def update_info(self, scroll_area,vertical_slider,horizontal_slider):
        rect = scroll_area.viewport().rect()
        top_left = horizontal_slider.value() , vertical_slider.value()
        center = top_left[0] + rect.width() // 2, top_left[1] + rect.height() // 2

        self.pos_label.setText(f"Top Left (x,y): {top_left}")
        self.center_label.setText(f"Center: {center}")
        self.size_label.setText(f"Crop: {rect.width()} x {rect.height()}")
        self.zoom_label.setText("Zoom: 100%")  # pode ser dinâmico

class ImageAnnotator(QMainWindow):
    def __init__(self):
        super().__init__()

        self.anysize_enabled = True
        self.anysize_controller = None
        self.anysize_X = 3840
        self.anysize_Y = 2160
        self.anysize_img_X = 0
        self.anysize_img_Y = 0
        self.global_x = 0
        self.global_y = 0
        self.anysize_X_zone = 1
        self.anysize_Y_zone = 1
        self.anysize_annotation_utils = AnnotationUtils(self)
    ...
    def activate_sam_magic_wand(self):
        ...
        self.sam_box_segmentation_button.setEnabled(True)
        self.sam_point_segmentation_button.setEnabled(True)
        ## Active Box as default mode
        self.sam_box_segmentation_button.setChecked(True)
        self.setSamBoxMode()
    ...
    def deactivate_sam_magic_wand(self):
        ...
        self.sam_box_segmentation_button.setEnabled(False)
        self.sam_box_segmentation_button.setChecked(False)
        self.sam_point_segmentation_button.setEnabled(False)
        self.sam_point_segmentation_button.setChecked(False)
    ...
    def setSamBoxMode(self):
        if self.sam_box_segmentation_button.isChecked():
            print("Box")
            self.image_label.setSamMode("box")
            self.sam_point_segmentation_button.setChecked(False)
        else:
            self.image_label.setSamMode(None)
    
    def setSamPointMode(self):
        if self.sam_point_segmentation_button.isChecked():
            print("Point")
            self.image_label.setSamMode("point")
            self.sam_box_segmentation_button.setChecked(False)
        else:
            self.image_label.setSamMode(None)
    ...
    def apply_sam_prediction(self):
        if self.image_label.sam_bbox is None and self.image_label.sam_point is None:
            print("SAM seed is None")
            return
        
        if self.image_label.sam_bbox:
            ...
            self.image_label.sam_bbox = None
            self.image_label.update()
        if self.image_label.sam_point:
            point = self.image_label.sam_point
            print(f"Applying SAM prediction with point: {point}")
        
            prediction = self.sam_utils.apply_sam_prediction(self.current_image, point)
            ...
            # Reset SAM bounding box
            self.image_label.sam_point = None
            self.image_label.update()
    
    def accept_sam_prediction(self):
        if self.image_label.temp_sam_prediction:
            new_annotation = self.image_label.temp_sam_prediction
            new_annotation['segmentation'] = self.convertLocalAnnotationToGlobal(new_annotation['segmentation'])
    ...
    def load_czi(self, image_path, dimensions=None, shape=None, force_dimension_dialog=False):
        ...
        # A ideia inicial aqui é, se a nossa solução estiver "ligada", começa a rodar um código nosso.

        if self.anysize_enabled:
            if self.image_label.pan_start_pos:
                self.image_label.pan_start_pos = None 
            self.anysize_controller = ImageController(image_path)
            sizes = self.anysize_controller.getImageSize()
            self.anysize_img_X = sizes["X"]
            self.anysize_img_Y = sizes["Y"]
            self.external_horizontal_slider.setMaximum(self.anysize_img_X)
            self.external_vertical_slider.setMaximum(self.anysize_img_Y)
            self.anysize_controller.crop(self.global_x,self.global_y,self.anysize_X,self.anysize_Y)
            self.info_panel.update_info(self.scroll_area,self.external_vertical_slider,self.external_horizontal_slider)
            image_array = self.anysize_controller.getOriginalCropped()
    ...
    def process_multidimensional_image(self, image_array, image_path, dimensions=None, force_dimension_dialog=False):
        ...
        if dimensions is None or force_dimension_dialog:
            if image_array.ndim > 2:
                if self.anysize_enabled:
                    default_dimensions = ['T','C','Z','H','W','W']
                else:
                    default_dimensions = ['Z', 'H', 'W'] if image_array.ndim == 3 else ['T', 'Z', 'H', 'W']
                    default_dimensions = default_dimensions[-image_array.ndim:]

                self.image_dimensions[base_name] = default_dimensions
                
                # A ideia é retirar esse pedaço do código e tirar o poder do usuário de escolher as dimensões do czi
    ...
    def export_annotations(self):
        ...
        self.removeEditTagFromAnnotations()
    
        if export_format == "COCO JSON":
            print(self.all_annotations)
    ...
    def load_image_annotations(self):
        ...
        if current_name in self.all_annotations:
            annotations = copy.deepcopy(self.all_annotations[current_name])
            self.updateLocalAnnotations(annotations)
    ...
    def save_current_annotations(self):
        ...
        if self.image_label.annotations:
            self.all_annotations[current_name] = self.updateGlobalAnnotations(self.image_label.annotations.copy())
            self.updateLocalAnnotations(copy.deepcopy(self.all_annotations[current_name]))
    ...
    def create_menu_bar(self):
        ...
        # Anysize Menu
        anysize_menu = menu_bar.addMenu("&Anysize")
        on_action = QAction("&On",self)
        on_action.triggered.connect(self.set_anysize_on)
        anysize_menu.addAction(on_action)
        off_action = QAction("&Off",self)
        off_action.triggered.connect(self.set_anysize_off)
        anysize_menu.addAction(off_action)
        
        
        
    
    def set_anysize_on(self):
        self.anysize_enabled = True
        print("Anysize ON")

    def set_anysize_off(self):
        self.anysize_enabled = False
        print("Anysize OFF")
    ...
    def setup_sidebar(self):
        ...
        # SAM-Assisted types buttons
        sam_types_layout = QHBoxLayout()
        self.sam_box_segmentation_button = QPushButton("SAM: Box Segmentation")
        self.sam_box_segmentation_button.setCheckable(True)
        self.sam_box_segmentation_button.setEnabled(False)
        self.sam_box_segmentation_button.clicked.connect(self.setSamBoxMode)
        self.sam_point_segmentation_button = QPushButton("SAM: Point Segmentation")
        self.sam_point_segmentation_button.setCheckable(True)
        self.sam_point_segmentation_button.setEnabled(False)
        self.sam_point_segmentation_button.clicked.connect(self.setSamPointMode)

        sam_types_layout.addWidget(self.sam_box_segmentation_button)
        sam_types_layout.addWidget(self.sam_point_segmentation_button)

        sam_layout.addLayout(sam_types_layout)  
    ...
    def setup_image_area(self):
        ...
        # Sliders internos (ocultos)

        self.scroll_area_vertical_bar = self.scroll_area.verticalScrollBar()
        self.scroll_area_horizontal_bar = self.scroll_area.horizontalScrollBar()

        # Conectando a o slider interno com as funções de atualização da imagem

        self.scroll_area_vertical_bar.valueChanged.connect(self.anysize_check_scroll_vertical)
        self.scroll_area_horizontal_bar.valueChanged.connect(self.anysize_check_scroll_horizontal)

        # Sliders Externos
        self.external_horizontal_slider = QSlider(Qt.Horizontal)
        self.external_vertical_slider = QSlider(Qt.Vertical)

        ## Conectando slider horizontal externo com o horizontal interno
        self.external_horizontal_slider.valueChanged.connect(self.setInternalScrollValueHorizontal)
        self.external_horizontal_slider.setMaximum(0)

        ## Conectando slider vertical externo com o vertical interno
        self.external_vertical_slider.valueChanged.connect(self.setInternalScrollValueVertical)
        self.external_vertical_slider.setMaximum(0)

        ## Estilizando slider vertical para ficar com a mesma lógica do horizontal (navegação de cima para baixo)
        self.external_vertical_slider.setInvertedAppearance(True)
        self.external_vertical_slider.setInvertedControls(True)

        ## Estilizando slider vertical para ter a mesma aparência do horizontal
        self.external_vertical_slider.setStyle(self.external_horizontal_slider.style())
        ...
        self.scroll_layout.addWidget(self.external_vertical_slider)

        self.image_layout.addLayout(self.scroll_layout)
        self.image_layout.addWidget(self.external_horizontal_slider)
    ...
    def anysize_check_scroll_vertical(self,value):
        if self.anysize_enabled:
            y_visivel = self.scroll_area.viewport().size().height()
            if value >= self.scroll_area_vertical_bar.maximum():
                if self.global_y + self.anysize_Y - y_visivel < self.anysize_img_Y:
                    self.global_y = min(self.global_y + self.anysize_Y - y_visivel, self.anysize_img_Y - self.anysize_Y)
                    self.load_czi(self.anysize_controller.getFilePath())
                    self.scroll_area_vertical_bar.setValue(1)    
            elif value <= 0:
                if self.global_y > 0:
                    self.global_y = max(self.global_y - self.anysize_Y + y_visivel, 0)
                    self.load_czi(self.anysize_controller.getFilePath())
                    self.scroll_area_vertical_bar.setValue(self.scroll_area_vertical_bar.maximum() - 1)
            self.info_panel.update_info(self.scroll_area,self.external_vertical_slider,self.external_horizontal_slider)

    def anysize_check_scroll_horizontal(self,value):
        if self.anysize_enabled:
            x_visivel = self.scroll_area.viewport().size().width()
            if value >= self.scroll_area_horizontal_bar.maximum():
                if self.global_x + self.anysize_X - x_visivel < self.anysize_img_X:
                    self.global_x = min(self.global_x + self.anysize_X - x_visivel, self.anysize_img_X - self.anysize_X)
                    self.load_czi(self.anysize_controller.getFilePath())
                    self.scroll_area_horizontal_bar.setValue(1)    
            elif value <= 0:
                if self.global_x > 0:
                    self.global_x = max(self.global_x - self.anysize_X + x_visivel, 0)
                    self.load_czi(self.anysize_controller.getFilePath())
                    self.scroll_area_horizontal_bar.setValue(self.scroll_area_horizontal_bar.maximum() - 1)
            self.info_panel.update_info(self.scroll_area,self.external_vertical_slider,self.external_horizontal_slider)
                
    def setInternalScrollValueVertical(self,value):
        newValue = value - self.global_y
        self.scroll_area_vertical_bar.setValue(newValue)

    def setInternalScrollValueHorizontal(self,value):
        newValue = value - self.global_x
        self.scroll_area_horizontal_bar.setValue(newValue)      
    ...
    def setup_image_list(self):
        ...
        self.image_list_layout.addWidget(QLabel("Navigation"))
        self.image_list_layout.addWidget(self.info_panel)
...
################################################################
    ...
    def IsAnnotationWithinCrop(self, annotation):
        ...
        w = self.anysize_X
        h = self.anysize_Y
        return self.anysize_annotation_utils.IsAnnotationWithinCrop(annotation, x, y, w, h)

    def convertGlobalAnnotationToLocal(self, annotation):
        top_x,top_y = self.anysize_controller.getOriginalTopLeft()
        return self.anysize_annotation_utils.convertAnnotations(annotation, -top_x, -top_y)
    
    def convertLocalAnnotationToGlobal(self, annotation):
        top_x,top_y = self.anysize_controller.getOriginalTopLeft()
        return self.anysize_annotation_utils.convertAnnotations(annotation, top_x, top_y)
    
    def updateLocalAnnotations(self,annotations):
        for class_name, class_annotations in annotations.items():
                for annotation in class_annotations:
                    if not self.IsAnnotationWithinCrop(annotation):
                        print("anotacao fora do crop")
                        annotation['edit'] = False
                        continue
                    else:
                        print("anotacao no crop")
                        annotation['edit'] = True
                        # Convert global segmentation to local coordinates
                        global_annotation = annotation["segmentation"]
                        local_annotation = self.convertGlobalAnnotationToLocal(global_annotation)
                        annotation["segmentation"] = local_annotation
        self.image_label.annotations = annotations
        
    def updateGlobalAnnotations(self,annotations):
        for class_name, class_annotations in annotations.items():
                for annotation in class_annotations:
                    if annotation.get('edit', False):
                        # Convert local segmentation to global coordinates
                        annotation['edit'] = True
                        local_annotation = annotation["segmentation"]
                        global_annotation = self.convertLocalAnnotationToGlobal(local_annotation)
                        annotation["segmentation"] = global_annotation
        return annotations
    
    def removeEditTagFromAnnotations(self):
        for file_name, file_annotations in self.all_annotations.items():
            for class_name, class_annotations in file_annotations.items():
                for annotation in class_annotations:
                    if 'edit' in annotation:
                        annotation.pop('edit',None)